...
public:
  ...
  char *cartaoLido() {
    if ((WiFi.status() == WL_CONNECTED)) {
      ...
      //postRequest(codigoRFIDLido);
      putRequest(codigoRFIDLido, "Beacon");
    }
    return (codigoRFIDLido);
  };

  void postRequest(String id) {
    Serial.println("Entrei");
    HTTPClient http;
    
    const char *url = "http://10.128.64.115:5500/device/add";
  ...
  void putRequest(String id, String newPosition) {
    Serial.println("Entrei");
    HTTPClient http;

    String base = "https://projeto3-client.vercel.app/api/devices/";
    String route = "/updatePosition";

    String test = "NB-05";

    String url = base + test + route;
